...
#class SessionsController < Devise::SessionsController
  ...
  #iOS:
  def sessionsUser
    @is_teacher = UserTeacher.select("user_teachers.*").where("user_teachers.id = ?",params[:id])
    #si no es maestro
    if @is_teacher == nil || @is_teacher == []
      ...
      @sessions = Session.select('sessions.*').where('user_id = ?', @user_teacher.user_id)
      @sessions.each do |u|
        if @user_student.score.to_i >= u.price.to_i
          u.avaible = 1
        else
          u.avaible = 0
        end
      ...
      @sessions = Session.select("sessions.*").where("sessions.user_id = ?",@is_teacher.first.user_id)
      @sessions.each do |u|
        u.avaible = 1
      end
    end
